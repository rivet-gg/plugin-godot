...
func setup_multiplayer():
	...
	if is_server:
		...
		# Notify lobby ready
		var response = await Backend.lobbies.set_lobby_ready({ "lobbyId": _lobby_id, "lobbyToken": _lobby_token }).async()
		if response.is_ok():
...
func _on_peer_disconnected(id):
	...
	if is_server:
		...
		if player_token != null:
			...
			var response = await Backend.lobbies.set_player_disconnected({
				"lobbyId": _lobby_id,
				"lobbyToken": _lobby_token,
...
func _auth_callback(id: int, buf: PackedByteArray):
	if is_server:
		...
		var response = await Backend.lobbies.set_player_connected({
			"lobbyId": _lobby_id,
			"lobbyToken": _lobby_token,
			"playerTokens": [player_token]
		}).async()
