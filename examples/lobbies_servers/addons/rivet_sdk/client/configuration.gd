...
#
# Generated at 2024-09-10T10:39:33.146Z

class_name RivetConfiguration
## Rivet configuration.
...
func _init():
	...
	RivetLogger.log('Endpoint: %s' % self.endpoint)
	self.game_version = _get_game_version()
	RivetLogger.log('Game version: %s' % self.game_version)
...
func _get_configuration():
	if FileAccess.file_exists(RIVET_DEPLOYED_CONFIGURATION_FILE_PATH):
		...
		if deployed_config_file and 'new' in deployed_config_file:
			RivetLogger.log('Using endpoint from deployed config')
	...
	if FileAccess.file_exists(RIVET_LOCAL_CONFIGURATION_FILE_PATH):
		...
		if deployed_config_file and 'new' in deployed_config_file:
			RivetLogger.log('Using endpoint from local config')
			return deployed_config_file.new()
...
func _get_backend_endpoint():
	...
	RivetLogger.log('Using default endpoint')
...
func _get_game_version():
	...
	if url_env:
		RivetLogger.log('Using game version from env')
		return url_env
