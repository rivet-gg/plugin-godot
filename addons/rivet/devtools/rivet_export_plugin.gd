...
func _export_begin(features: PackedStringArray, is_debug: bool, path: String, flags: int) -> void:
	...
	var configuration_file = FileAccess.open(
		RivetPluginBridge.RIVET_CONFIGURATION_FILE_PATH, FileAccess.READ
	)
	var source = configuration_file.get_as_text()
	var script = GDScript.new()
	script.source_code = source
	var error: Error = ResourceSaver.save(
		script, RivetPluginBridge.RIVET_DEPLOYED_CONFIGURATION_FILE_PATH
	)
	if not error:
		has_added_file = true
