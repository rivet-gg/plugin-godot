...
func prepare() -> void:
	InstallLabel.add_theme_font_override(
		&"mono_font", get_theme_font(&"output_source_mono", &"EditorFonts")
	)
	...
	InstallLabel.text = (
		InstallLabel
		. text
		. replace(&"%%version%%", RivetPluginBridge.get_plugin().cli.REQUIRED_RIVET_CLI_VERSION)
		. replace(&"%%bin_dir%%", RivetPluginBridge.get_plugin().cli.get_bin_dir())
	)
...
func _on_install_button_pressed() -> void:
	...
	if result.exit_code == 0:
		...
		if not error:
			...
			InstallDialog.dialog_text = (
				&"Rivet installed successfully!\nInstalled Rivet %s in %s"
				% [
					RivetPluginBridge.get_plugin().cli.REQUIRED_RIVET_CLI_VERSION,
					RivetPluginBridge.get_plugin().cli.get_bin_dir()
				]
			)
	...
	InstallDialog.dialog_text = (
		&"Rivet installation failed! Please try again.\n\n%s" % result.output
	)
	InstallDialog.popup_centered()
	InstallButton.loading = false
