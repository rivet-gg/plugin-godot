...
static func _get_configuration():
	...
	if FileAccess.file_exists(RivetPluginBridge.RIVET_DEPLOYED_CONFIGURATION_FILE_PATH):
		var deployed_config_file = ResourceLoader.load(
			RivetPluginBridge.RIVET_DEPLOYED_CONFIGURATION_FILE_PATH
		)
...
static func POST(owner: Node, path: String, body: Dictionary) -> RivetRequest:
	...
	return RivetRequest.new(
		owner, HTTPClient.METHOD_POST, url, {"headers": _build_headers(service), "body": body_json}
	)
...
static func GET(owner: Node, path: String, body: Dictionary) -> RivetRequest:
	...
	return RivetRequest.new(
		owner, HTTPClient.METHOD_GET, url, {"headers": _build_headers(service), "body": body_json}
	)
