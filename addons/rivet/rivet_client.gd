...
func player_disconnected(body: Variant, on_success: Callable, on_fail: Callable):
	_rivet_request_with_body(
		"POST", "matchmaker", "/players/disconnected", body, on_success, on_fail
	)
...
## @deprecated
func _rivet_request(
	method: String, service: String, path: String, on_success: Callable, on_fail: Callable
):
...
func _http_request_completed(
	result, response_code, _headers, body, on_success: Callable, on_fail: Callable
):
